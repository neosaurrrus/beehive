...
import { ButtonLabel, Heading1, Heading2, Paragraph } from "@/components/Text/Text"
...
export default function Header({ game, players, currentPlayer }: { game: Game, players: Player[], currentPlayer: Player }) {
  ...
  const renderAdminButtons = () => (
    <div className={`flex flex-col gap-4 justify-center items-center duration-200 ${hasSomeCardsPlayed ? 'opacity-100' : 'opacity-50'}`}>
      <button
  ...
  const buttonClasses = `bg-accent/75 flex justify-center items-center p-4 rounded-4xl shadow-lg w-32 h-12 ${hasSomeCardsPlayed && 'hover:bg-accent'} duration-200`
